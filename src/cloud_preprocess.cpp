...
void Preprocess::livox_handler(const sensor_msgs::PointCloud2::ConstPtr &msg) {
  ...
  for (uint i = 1; i < plsize; i++) {
    if ((pl_orig.points[i].line < N_SCANS) &&
      ...
      if (i % point_filter_num == 0) {
        ...
        pl_full[i].curvature =
            static_cast<float>(pl_orig.points[i].timestamp - timebase) /
  ...
  for (uint i = 1; i < plsize; i++) {
    if (is_valid_pt[i]) {
      pl_surf.emplace_back(pl_full[i]);
    }
  }
}
